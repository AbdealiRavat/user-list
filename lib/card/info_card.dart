  ...
  Widget build(BuildContext context) => Obx(() {
    ...
    return ListView.builder(
      ...
      itemBuilder: (BuildContext context, int index) {
        final imgUrl = apiController.post[index]['profileImage']; // Extract image URL
        final name = apiController.post[index]['name']; // Extract name
        final email = apiController.post[index]['email']; // Extract email
        final phone = apiController.post[index]['phone']; // Extract phone number
        ...
        return InkWell(
          onTap: () {
            ...
            Navigator.push(context, MaterialPageRoute(builder: (context) => UserDetails(name: name, email: email, imgUrl: imgUrl,)));
          },
          child: CardContainer(
            child: Row(
